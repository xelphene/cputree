...
function buildMioMerge2(dstBranch, srcBranch, inputMode)
{
    ...
    if( ! dstBranch[mioMeta].hasOwnProperty(mioNegate) )
    {
        ...
        var srcSym = dstBranch[mioMeta][mioNegate].srcSym;
        var inSym  = dstBranch[mioMeta][mioNegate].inSym;
        var outSym = dstBranch[mioMeta][mioNegate].outSym;
    }
    
    if( ! dstBranch.parent.hasc(srcSym) )
    ...
    if( srcBranch.isRoot )
        ...
        dstBranch.parent.getc(srcSym).merge( srcBranch );
    
    fillOtherLeaf(dstBranch, dstBranch.parent.getc(srcSym) );
    ...
    if( inputMode=='link' ) {
        ...
        fillInputLeaf(dstBranch, dstBranch.parent.getc(srcSym) );
    } else if( inputMode=='replace' ) {
        replaceInputLeaf(dstBranch, dstBranch.parent.getc(srcSym) );
    } else
